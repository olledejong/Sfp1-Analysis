...
def main():
    ...
    for pos in range(1, 21):
        ...
        for cell in [i for i in individual_cells if pos in i]:
            ...
            single_cell_data = single_cell_data.sort_values(by="Time")
            ...
            for t in single_cell_data.TimeID:
                ...
                # fit an ellipse on the whole cell mask
                ...
                r3c = (np.average((c_MA, c_ma)) * scaling_factor)  # to get the third axis
                cell_volume = (4 / 3) * pi * r1c * r2c * r3c
                cell_area = pi * r1c * r2c

                cell_areas.append(cell_area)  # save the area
                cell_volumes.append(cell_volume)  # save the volume for this datapoint
                ...
                r3n = (np.average((MA, ma)) * scaling_factor)  # to get the third axis
                nuc_volume = (4 / 3) * pi * r1n * r2n * r3n
                nuc_area = pi * r1n * r2n

                # save this time-point its area and volume of the nucleus
            ...
            ratios = []
            i = 0
            for nuc_vol in nuc_volumes:
                if nuc_vol is None:
                    ratios.append(None)
                else:
                    ratios.append(nuc_vol / cell_volumes[i])
    ...
    final_dataframe.to_excel(f"{output_dir}excel/nup133_final_data_script_test.xlsx")  # save the final file
    print("Done.")
